...
// This function will be used in the loop function to handle all alarm scenarios as seen below
void alarm()
{
  ...
  if (disabled)
  {
    ...
    if (buttonState == 0) // If the button is pressed reset the alarm
    {
      Serial.print("\nALARM RESET");
      disabled = false;
      delay(BTN_DELAY); // This delay helps the program read the button status
    }
  }
